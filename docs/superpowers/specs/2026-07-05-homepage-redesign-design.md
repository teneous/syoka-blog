...
- Header 不动(wordmark 来自 `siteMetadata.headerTitle` = "Syoka Studio"),但移除右上角的 Trend 按钮(`components/navigation/Trend.tsx` 已删除),其余保持不变。
...
| H1      | 从业务痛点，到可交付的解决方案(纯文本，无高亮衬底)                                                                 | From business pain points to shippable solutions.(plain text, no highlight)                                                                                                                                          |
| 副标题  | 我是 Syoka，长期深耕 Agent、系统架构、基建、数据工程。喜欢深入业务场景，把复杂问题做成简洁、可靠、持续演进的产品。 | I'm Syoka, focused long-term on agents, system architecture, AI infrastructure, and data engineering. I dig deep into business contexts and turn complex problems into simple, reliable products that keep evolving. |
...
| 基建横杆    | AI 基建 / Skill · MCP · CLI / ↑ 支撑上层应用          | AI Infrastructure / Skill · MCP · CLI / ↑ Supports the apps above                        |
...
| 系统架构(Braces,青)   | 为系统定边界、立规则，让架构支撑业务的长期演进。       | 微服务架构 · 数据架构 · 系统设计   | Set boundaries and rules so architecture supports long-term evolution.          | Microservices · Data architecture · System design |
| 基建(Cpu,紫)          | 把 AI 能力平台化，让多个业务场景持续、低成本地接入。   | Skill · MCP · CLI                  | Turn AI capability into a platform many scenarios can plug into.                | Skill · MCP · CLI                                 |
